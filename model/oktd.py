...
class OKTD(object):
    ...
    #         selections = np.ones_like(kernel_state)
    #         # print(selections)
    #         return selections
    ...
    def cal_value(self, kernels):
        if self.states_dict is None:
            return 0
        else:
            return np.dot(self.theta, kernels)
    ...
    def epsilon_greedy(self, observation):
        if np.random.uniform() > self.epsilon:
            ...
            for i in range(self.n_actions):
                ...
                q_value[i] = reward + self.gamma * self.cal_value(kernels_) * np.abs(done - 1)
    ...
    def learn(self, observation, action, reward, observation_, done):
        # selections = self.selection(norm_observation)
        kernels = self.kernels(observation)
        # selections_ = self.selection(norm_observation_)
        kernels_ = self.kernels(observation_)
        if done:
            delta = reward - self.cal_value(kernels)
        else:
            delta = reward + self.gamma * self.cal_value(kernels_) - self.cal_value(kernels)
        nabla_theta = kernels
    ...
    def run(self, train_env, test_env, train_steps, test_steps, episodes, path, seed):
        ...
        for t in range(train_steps):
            ...
            if (t + 1) % test_steps == 0:
                ...
                print("OKTD: After Training {}k Steps "
                      "\tDictionary memory: {}"
