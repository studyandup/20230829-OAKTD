...
parser.add_argument("--env", default="PuddleWorldEnv", type=str, help="env name")
...
def train(agent, env, env1, n_episodes, test_steps, train_steps, eps_start, eps_end, eps_decay, path,
    ...
    f2 = path1 + "/MountainCar-" + describe + ".csv"
    ...
    for t in range(train_steps):
        ...
        if (t + 1) % test_steps == 0:
            test_scores = test(agent, env1, n_episodes, eps)
...
def begin():
    ...
    path1 = str(base_dir) + '/rewards/' + env_name + '/' + alg

    env = Env["PuddleWorldEnv"]()
    env1 = Env["PuddleWorldEnv"]()
